...
import {
  getAllBarbers, createBarber, deleteBarberByUsername,
  updateBarber, type BarberCreate, type ServiceItem, type BarberUpdate
...
export default function BarbersAdmin() {
  ...
  const [form, setForm] = useState<BarberCreate>({
    firstname: "", lastname: "", username: "", phone: "", age: "",
    email: "", password: "", servicesOffered: [{ name: "Haircut", duration: 30, price: 20 }],
  });
  ...
  async function load() {
    ...
    try {
      ...
      setErr(e?.message ?? "Failed to load barbers");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(e?.message ?? "Failed to create barber");
    }
  }
  ...
  async function onDelete(username: string) {
    ...
    try {
      ...
      setErr(e?.message ?? "Failed to delete barber");
    }
  }
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">
                      {(b.firstname || b.username) + (b.lastname ? ` ${b.lastname}` : "")}
                    </p>
                    <p className="text-sm text-gray-600">{b.username} • {b.email}</p>
                  </div>
                  <div className="flex gap-3">
                    <button onClick={() => setEdit(b)} className="text-blue-600 hover:text-blue-700">Edit</button>
                    <button onClick={() => onDelete(b.username)} className="text-red-600 hover:text-red-700">Delete</button>
                  </div>
                </div>
      ...
      <div>
        ...
        <form onSubmit={onCreate} className="space-y-3 rounded-2xl border p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input placeholder="First name" className="border rounded-lg px-3 py-2"
              value={form.firstname} onChange={e => setForm(f => ({ ...f, firstname: e.target.value }))} />
            <input placeholder="Last name" className="border rounded-lg px-3 py-2"
              value={form.lastname} onChange={e => setForm(f => ({ ...f, lastname: e.target.value }))} />
            <input placeholder="Username" className="border rounded-lg px-3 py-2"
              value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} />
            <input placeholder="Email" className="border rounded-lg px-3 py-2"
              value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
            <input placeholder="Phone" className="border rounded-lg px-3 py-2"
              value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
            <input placeholder="Age" className="border rounded-lg px-3 py-2"
              value={form.age} onChange={e => setForm(f => ({ ...f, age: e.target.value }))} />
          </div>
          <input type="password" placeholder="Password" className="border rounded-lg px-3 py-2 w-full"
          ...
          <div className="space-y-2">
            ...
            {form.servicesOffered.map((s, i) => (
              <div key={i} className="grid grid-cols-1 md:grid-cols-3 gap-2">
                ...
                <input placeholder="Duration (min)" type="number" min={1} className="border rounded-lg px-3 py-2"
                  value={s.duration} onChange={e => setSvc(i, { duration: Number(e.target.value) })} />
                <input placeholder="Price (€)" type="number" min={0} className="border rounded-lg px-3 py-2"
          ...
          <button type="submit" className="bg-blue-600 text-white px-5 py-2 rounded-lg">Create Barber</button>
        </form>
      </div>
    </div>
  );
